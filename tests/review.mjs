...
export async function runReview(browser, base, t) {
  ...
  try {
    ...
    t.ok('признак PS5 Pro доехал до промпта', /ЧЁРНАЯ ВСТАВКА С ВЕНТИЛЯЦИОННЫМИ ПРОРЕЗЯМИ/.test(vs));
    // Признак — наличие прорезей, а не их количество: модель насчитала «одну
    // секцию» вместо трёх полос и из-за этого не назвала Pro
    t.ok('количество полос не требуется считать', /НЕ считай их/.test(vs));
    ...
    t.ok('подсказка приехала из дескриптора категории, а не из кода промпта',
      await page.evaluate(() => BUILTIN_CATS.find(c => c.id === 'phys').visionHint.includes('ВЕНТИЛЯЦИОННЫМИ ПРОРЕЗЯМИ')));
