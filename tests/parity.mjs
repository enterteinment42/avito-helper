...
//
// ⚠️ ВАЖНО: baseline — это git HEAD, он едет вперёд с каждым коммитом. Списки
// ниже описывают отличия от ПОСЛЕДНЕГО коммита, поэтому сразу после того, как
// изменение закоммичено, их надо ОЧИСТИТЬ — иначе стенд начнёт вычитать из
// нового промпта строку, которая уже есть и в эталоне, и паритет упадёт на
// ровном месте. Само поведение при этом закрепляется обычным тестом в своей
// секции (правило selling_word живёт в review.mjs), а не этим списком.
...
export async function runParity(browser, base, t) {
  ...
  try {
    ...
    for (const id of [...ids, '__none']) {
      ...
      t.ok(`[SYS ${id}] системный промпт идентичен${EXPECTED_SYS_LINES.length ? ' (кроме заявленных строк)' : ''}`,
        stripped === sysB[id], `длины: новый ${stripped.length}, старый ${sysB[id]?.length}`);
      if (EXPECTED_SYS_LINES.length) {
