...
export async function runPhys(browser, base, t) {
  ...
  try {
    ...
    // Находка код-ревью: «оформляешь за X руб» противоречило правилу категории
    t.ok('запрет ярлыка «цена» не предписывает «оформляешь»', !/оформляешь за X руб/.test(sys),
      sys.split('\n').filter(l => /Слово «цена» как ценник/.test(l)).join(''));
    t.ok('но сам ярлык «цена» по-прежнему запрещён', /Слово «цена» как ценник/.test(sys));
    t.ok('в игре формулировка прежняя',
      /оформляешь за X руб/.test(await page.evaluate(() => buildSys('game'))));
    ...
    // Категория запрещает цену в тексте — значит и на обложку она не идёт, иначе
    // Сканер ругался бы на цифру, которую сам же инструмент туда и нарисовал
    t.ok('цены на обложке нет — она запрещена правилом категории',
      !infog.some(l => /₽/.test(l)), JSON.stringify(infog));
